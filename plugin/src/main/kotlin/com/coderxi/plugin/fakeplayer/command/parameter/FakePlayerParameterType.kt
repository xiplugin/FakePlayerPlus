...
class FakePlayerParameterType(private val fpm : FakePlayerManager) : ParameterType<BukkitCommandActor, FakePlayer> {
    ...
    override fun parse(input: MutableStringStream, context: ExecutionContext<BukkitCommandActor>): FakePlayer? {
        ...
        if (sender is Player) {
            if (name.isEmpty()) {
                val current = sender.selected
                if (current != null) return current
                val myPlayers = fpm.fakeplayersByOwnerUuid(sender.uniqueId)
                if (myPlayers.size == 1) {
                    sender.selected = myPlayers.first()
                    return myPlayers.first()
                }
                throw NoSelectedException()
            }
